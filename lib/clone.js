...
// private virtualenv on first use; nothing Python-related is bundled with Audiobard itself.
...
const markerFile = (venvDir) => path.join(venvDir, '.audiobard-ok');
...
class CloneEngine {
  constructor({ venvDir, script, onStatus = () => {}, onLog = () => {} }) {
    Object.assign(this, { venvDir, script, onStatus, onLog, pending: new Map(), nextId: 1, ready: null, child: null, refPath: null, refSent: false });
  }
  ...
  start() {
    ...
    this.ready = new Promise((resolve, reject) => {
      ...
      this.refSent = false;
      let settled = false;
      // A write to a sidecar that has just died raises EPIPE; the pending request is rejected from 'close'.
  ...
  request(payload) {
    return new Promise((resolve, reject) => {
      if (!this.child) return reject(new Error('Voice cloning engine is not running'));
  ...
  async setReference(wavPath) {
    await this.start();
    this.refPath = wavPath;
    this.refSent = false;
    await this.request({ op: 'ref', path: wavPath });
    this.refSent = true;
  }

  // The sidecar is started again after a crash, and the new process knows nothing of the
  // reference sample, so it is sent once more before the first synthesis on each process.
  async synth(text, out) {
    await this.start();
    if (!this.refSent) {
      if (!this.refPath) throw new Error('No voice sample has been set.');
      await this.request({ op: 'ref', path: this.refPath });
      this.refSent = true;
    }
    return this.request({ op: 'synth', text, out });
  }
  stop() {
