...
class ChapterEncoder {
  constructor({ ffmpeg, format, file, rate }) {
    ...
    this.exit = new Promise((resolve) => {
      this.child.on('error', (e) => { this.err += e.message; resolve(-1); });
...
// Length of an encoded chapter, for resuming. ffprobe is the proper tool, but some ffmpeg
// installs ship without it, so the "Duration:" line ffmpeg prints on a bare -i is the fallback.
function probeSeconds(file, ffmpeg = 'ffmpeg') {
  const r = spawnSync('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', file], { encoding: 'utf8' });
  const s = parseFloat(r.stdout);
  if (r.status === 0 && Number.isFinite(s)) return s;
  const m = spawnSync(ffmpeg, ['-hide_banner', '-i', file], { encoding: 'utf8' }).stderr?.match(/Duration:\s*(\d+):(\d+):(\d+(?:\.\d+)?)/);
...
function concatM4b({ ffmpeg, parts, out, title, author, onLine = () => {} }) {
  ...
  const listFile = path.join(dir, '.audiobard-concat.txt');
  const metaFile = path.join(dir, '.audiobard-meta.txt');
