...
Pick an EPUB, MOBI or AZW3, choose a voice, press **Create audiobook**. Audiobard reads the book aloud
with a built-in neural voice, or in the voice of anyone who gives you a short recording.

<p align="center">
  <img src="docs/screenshot.png" alt="Audiobard with Alice's Adventures in Wonderland loaded, ready to narrate" width="620">
</p>
...
- `ffmpeg` on the PATH, for M4B, MP3 and OGG output. Without it Audiobard falls back to WAV, which needs about 170 MB per hour of audio.
...
Produces `dist/Audiobard-<platform>-<arch>/` with an `audiobard` executable inside. The packaging script
...
In the app, choose **Clone a voice from a sample** and press **Set up voice cloning**. Audiobard creates
...
If voice cloning was set up before a GPU was available, or on a version that only knew
...
`10.3.0` for RDNA2) in the environment Audiobard starts from usually fixes it, and the variable is
passed through to the engine. On macOS the standard wheel is installed and cloning runs on the CPU.

Audiobard downloads only the three weight files the Turbo model reads, about 3 GB, rather than the
...
lib/migrate.js        One-time move of data out of the data directories of earlier versions
python/audiobard_tts.py Chatterbox Turbo behind a JSON-lines protocol on stdin/stdout
...
`<userData>` is `~/.config/Audiobard` on Linux (`$XDG_CONFIG_HOME/Audiobard` if that variable is
set), `~/Library/Application Support/Audiobard` on macOS and `%APPDATA%\Audiobard` on Windows. The
CLI uses the same directory as the app, so models and the voice cloning setup are shared.

The app has had other names before 1.3.0, each with its own data directory. On first start, the
models, Python environment and recorded samples are moved out of those directories, so nothing is
downloaded or set up twice.
...
sample for you knowing what it is for. Audiobard is built for narrating books you own in a voice
...
Audiobard is licensed under the [PolyForm Noncommercial License 1.0.0](LICENSE.md). You may use,
