...
// Paragraphs shorter than this (chapter numbers, "* * *", lone headings) are folded into the
// next paragraph: on their own they make TTS models stumble or ramble.
const MIN_PARA = 20;
...
function paragraphs(text) {
  ...
  for (const para of normalize(text).split(/\n\s*\n/)) {
    ...
    if (p.length < MIN_PARA || !/\p{L}/u.test(p)) {
      pending = pending ? `${pending} ${p}` : p;
      continue;
    }
    out.push(pending ? `${/[.!?:;,]$/.test(pending) ? pending : `${pending}.`} ${p}` : p);
    pending = '';
  }
  if (pending) {
