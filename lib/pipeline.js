...
const { WavWriter, silence, readWav, wavSeconds, floatToPcm16 } = require('./wav');
...
const safeName = (s) => (s || 'Untitled').replace(/[<>:"/\\|?*]+/g, '-').replace(/\s+/g, ' ').trim().slice(0, 80).replace(/[. ]+$/, '') || 'Untitled';
...
async function getSynth(job, emit) {
  ...
  cloneEngine.onLog = (l) => emit({ type: 'log', message: l });
  await cloneEngine.setReference(job.refAudio);
  return async (text) => {
    const tmp = path.join(os.tmpdir(), `audiobard-${process.pid}-${tmpCounter++}.wav`);
    try {
...
async function convert(job, emit, isCancelled) {
  ...
  const existingSeconds = (file) => (format.codec ? probeSeconds(file, job.ffmpeg) : wavSeconds(file));
